...
def init_logger(
    log_name=REPO_NAME,
    log_file: Path = Path(PROJECT_PATH, "logs", f"{REPO_NAME}.log"),
) -> logging.Logger:
    ...
    # when passing __file__, set to caller basename (no extension)
    logger = logging.getLogger(name=Path(log_name).stem)
